...
#Projekt: Rundenzeiten, graphische Benuteroberfläche 
...
#fenster 6 ende
    ...
    def rennstart():
        ...
        x = s.recv(1024)#Da die beiden Autos beim Start jeweils durch die beiden Schranken fahren und in der Arduino Datei dabei aber Zeiten versendet werden,
        y = s.recv(1024)#müssen diese Zeiten im Code auch empfangen und in diesem Fall verworfen werden
        x = s.recv(1024)#Deshalb wird hier viermal ein Wert empfangen aber nicht benötigt
        y = s.recv(1024)


    
        while (k+z)<12:
            x = s.recv(1024)#Python empfängt zuerst die Rundenzeit vom Arduino
    
            if (float (x) > 100000):
                if(k<6):
                    x = float (x) - 100000 #100000 wird abgezogen um auf die Ursprungszahl zu kommen
                    x = (float (x)/1000)#Der empfangene Wert für die Rundenzeit wird von Millisekunden in Sekunden umgerechnet
                    y = s.recv(1024)#Python empfängt nun die Zeit, zur Berechnung der Geschwindigkeit
                    ms = (0.115/(float (y)/1000))#Für die Geschwindigkeit gilt die Formel: Strecke/Zeit Die Strecke beträgt 11,5 cm (=0,115 m)
                                                 #und die Zeit wird erneut durch 1000 geteilt um den Wert in Sekunden nicht in Millisekunden zu erhalten
                    kmh = (ms*3.6)               #Um den km/h Wert zu erhalten muss der Wert für Meter/Sekunde mit 3,6 multipliziert werden
